...
from reachy_robotis.audio.speech_tapper import HOP_MS, SwayRollRT
...
class HeadWobbler:
    ...
    def __init__(self, set_speech_offsets: Callable[[Tuple[float, float, float, float, float, float]], None]) -> None:
        ...
        self._motion_scale = self._read_motion_scale()
        self._motion_interval_s = self._read_motion_interval_s()
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                while i < len(results):
                    ...
                    r = results[i]
                    offsets = (
                        r["x_mm"] / 1000.0 * self._motion_scale,
                        r["y_mm"] / 1000.0 * self._motion_scale,
                        r["z_mm"] / 1000.0 * self._motion_scale,
                        r["roll_rad"] * self._motion_scale,
                        r["pitch_rad"] * self._motion_scale,
                        r["yaw_rad"] * self._motion_scale,
                    )
                    ...
                    if now - self._last_apply_ts >= self._motion_interval_s:
    ...
    def _read_motion_scale(self) -> float:
        raw = os.getenv("REACHY_MINI_SPEECH_MOTION_SCALE", "1.0")
        try:
            return max(0.0, min(1.0, float(raw)))
        except ValueError:
            logger.warning("Invalid REACHY_MINI_SPEECH_MOTION_SCALE=%r; using 1.0", raw)
            return 1.0

    def _read_motion_interval_s(self) -> float:
        raw = os.getenv("REACHY_MINI_SPEECH_MOTION_HZ", "2.0")
        try:
            hz = float(raw)
        except ValueError:
            logger.warning("Invalid REACHY_MINI_SPEECH_MOTION_HZ=%r; using 2.0", raw)
            hz = 2.0
        if hz <= 0:
            return float("inf")
        return 1.0 / min(hz, 20.0)
