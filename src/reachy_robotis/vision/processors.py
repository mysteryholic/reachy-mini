...
try:
    ...
    class _MissingTorch:
        cuda = _MissingCuda()
        mps = _MissingMps()
        backends = type("_Backends", (), {"mps": _MissingMps()})()
        float32 = "float32"
        bfloat16 = "bfloat16"
...
try:
    ...
    class _MissingAutoClass:
        @staticmethod
        def from_pretrained(*_args: Any, **_kwargs: Any) -> Any:
            raise RuntimeError("transformers is not installed")

    AutoProcessor = _MissingAutoClass  # type: ignore[assignment]
    AutoModelForImageTextToText = _MissingAutoClass  # type: ignore[assignment]
