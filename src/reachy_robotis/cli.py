...
def test_task_command(task_name: str, dry_run: bool = False) -> int:
    ...
    print(f"\n{'TEST MODE' if dry_run else 'EXECUTING'}: {task.name}")
    print("-" * 80)

    if task.device != "mock":
        print(f"\nWARNING: This task uses device: {task.device}")
    ...
    try:
        while True:
            ...
            if user_input.lower() == "status":
                ...
                print(f"   Success: {'yes' if snapshot.get('last_execution_ok') else 'no'}")
...
ROBOT CONTROL:
  ...
  Current state:
    ...
    EXECUTING: Tasks run when triggered
