...
async def main() -> None:
    ...
    bad_terminal = await manager.start_terminal("omx_bringup_f", "does_not_exist", dry_run=True)
    assert not bad_terminal["ok"] and bad_terminal["error"] == "unknown_terminal", bad_terminal
    started = await manager.start_terminal("omx_bringup_f", "omx_bringup_f", dry_run=True)
